...
6. 每次开发任何新特性前，必须先执行 `git pull origin main --rebase`，再从同步后的代码开始拆分任务。
7. 前端与后端的开发分支职责不得重叠：
   - 前端分支只提交前端相关改动与必要文档更新
   - 后端分支只提交后端相关改动与必要文档更新
   - 若一个特性需要双端并行开发，应由 `Coordinator Agent` 负责契约与最终集成，不得让前后端分支直接交叉覆盖彼此文件
...
5. 任何准备开始的新特性，如果未先完成 `git pull origin main --rebase`，则不得进入实现阶段。
